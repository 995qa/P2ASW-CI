...
bool CSysSessionBase::Update()
{
	...
	if ( !IsServiceSession() )
	{
		...
		CSteamID idRemote;
		// Swarm's version of SteamAPI does not let you specify the channel here - hope it's not important
		// Is there any way we can see what the original Swarm matchmaking DLL is doing?
		while( steamapicontext->SteamNetworking()->IsP2PPacketAvailable( &uiSteamMsgSize
#ifndef P2ASW
																		 , INetSupport::SP2PC_LOBBY 
#endif
		))
		{
			utlMemory.EnsureCapacity( uiSteamMsgSize );
			if ( steamapicontext->SteamNetworking()->ReadP2PPacket( utlMemory.Base(), uiSteamMsgSize, &uiSteamMsgSize, &idRemote
#ifndef P2ASW
																	, INetSupport::SP2PC_LOBBY 
#endif
			))
...
void CSysSessionBase::Destroy()
{
	...
	{
#ifndef P2ASW // Swarm SteamAPI does not have CloseP2PChannelWithUser()
		if ( !IsServiceSession() )
		{
...
// 		}
		for ( int k = 0, numMachines = m_pSettings->GetInt( "members/numMachines" ); k < numMachines; ++ k )
		{
			for ( int ic = 0, numPlayers = m_pSettings->GetInt( CFmtStr( "members/machine%d/numPlayers", k ) ); ic < numPlayers; ++ ic )
			{
				...
				{
					steamapicontext->SteamNetworking()->SendP2PPacket( xuidPlayer, buf.Base(), buf.TellMaxPut(), eSendType
#ifndef P2ASW
																	   , INetSupport::SP2PC_LOBBY 
#endif
					);
...
char const * CSysSessionBase::LobbyEnterErrorAsString( LobbyEnter_t *pLobbyEnter )
{
	...
	{
	// Portal 2 does this, kind of stupid...
...
#ifdef P2ASW
		// Swarm SteamAPI doesn't have k_ELobbyTypePrivate, but it's 0 internally, hopefully this works
		return *peType = (ELobbyType)0, true; // private
#else
