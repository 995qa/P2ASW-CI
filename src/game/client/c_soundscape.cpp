...
int C_SoundscapeSystem::AddLoopingSound( const char *pSoundName, bool isAmbient, float volume, soundlevel_t soundlevel, int pitch, const Vector &position, float radius, float flFadeRate )
{
	...
	if ( soundSlot < 0 )
	{
		...
		if ( isAmbient )
		{
			...
			enginesound->EmitAmbientSound( pSoundName, 0, pitch, ShouldPauseAllSounds() ? SND_SHOULDPAUSE : 0 );
...
void C_SoundscapeSystem::UpdateLoopingSound( loopingsound_t &loopSound )
{
	...
	if ( loopSound.isAmbient )
	{
		enginesound->EmitAmbientSound( loopSound.pWaveName, loopSound.m_volume.m_flCurrent, loopSound.pitch, 
									   ShouldPauseAllSounds() ? SND_CHANGE_VOL | SND_SHOULDPAUSE : SND_CHANGE_VOL);
	}
	else
	{
		CLocalPlayerFilter filter;
...
void C_SoundscapeSystem::PlayRandomSound( randomsound_t &sound )
{
	...
	if ( sound.isAmbient )
	{
		enginesound->EmitAmbientSound( pWaveName, sound.masterVolume * RandomInterval( sound.volume ), (int)RandomInterval( sound.pitch ), ShouldPauseAllSounds() ? SND_SHOULDPAUSE : 0 );
	}
	else
	{
		CLocalPlayerFilter filter;
